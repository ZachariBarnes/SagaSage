resource "aws_lambda_function" "save_char_lambda" {
  function_name = "save_character_lambda"
  filename      = local.output_path
  role          = aws_iam_role.save_lambda_exec.arn
  runtime       = "nodejs18.x"
  handler       = "index.saveNewCharacter" # <file-name>.<fucntion-name> 
  timeout       = local.lambda_timeout
  depends_on    = [aws_iam_role_policy_attachment.save_lambda_policy_attachment]
  ...
  environment {
    variables = {
      ...
      S3_IMAGE_BUCKET                 = local.env["S3_IMAGE_BUCKET"],
      AWS_CREDENTIAL_DURATION_SECONDS = local.env["AWS_CREDENTIAL_DURATION_SECONDS"],
      AWS_SAVE_ROLE_ARN               = local.env["AWS_SAVE_ROLE_ARN"],
    }
  }
}
...
data "archive_file" "save_char_lambda" {
  type = "zip"
...
# resource "aws_lambda_permission" "save_char_lambda_apigateway_permission" {
#   statement_id  = "AllowAPIGatewayInvoke"
#   action        = "lambda:InvokeFunction"
#   function_name = aws_lambda_function.save_char_lambda.function_name
#   principal     = "apigateway.amazonaws.com"
#   # source_arn = "${aws_api_gateway_rest_api.main.execution_arn}/*/*"
#   source_arn    = "arn:aws:execute-api:${local.region}:${local.account_id}:${aws_api_gateway_rest_api.main.id}/*/*"
# }

resource "aws_lambda_function_url" "save_character_dev" {
  function_name = aws_lambda_function.save_char_lambda.function_name
  # qualifier          = "$LATEST"
  authorization_type = "NONE"
  depends_on         = [aws_lambda_function.save_char_lambda]
...
output "save_char_lambda_base_url" {
  value = "${aws_lambda_function_url.save_character_dev.function_url}/"
}
