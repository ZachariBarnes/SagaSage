resource "aws_lambda_function" "portrait_lambda" {
  function_name = "portrait_lambda"
  filename      = local.output_path
  role          = aws_iam_role.character_lambda_exec.arn
  runtime       = "nodejs18.x"
  handler       = "index.portrait" # <file-name>.<fucntion-name> 
  ...
  environment {
    variables = {
      OPENAI_API_ORG         = local.env["OPENAI_API_ORG"],
      OPENAI_API_KEY         = local.env["OPENAI_API_KEY"],
      FAKE_OPENAI_API_KEY    = local.env["FAKE_OPENAI_API_KEY"],
      AUTH_KEY               = local.env["AUTH_KEY"],
      TBFM_URL               = local.env["TBFM_URL"],
      TBFM_AUTH              = local.env["TBFM_AUTH"],
      API_HOST               = local.env["API_HOST"],
      COMFY_UI_URL           = local.env["COMFY_UI_URL"],
...
data "archive_file" "portrait_lambda" {
  type = "zip"
...
# resource "aws_lambda_permission" "portrait_lambda_apigateway_permission" {
#   statement_id  = "AllowAPIGatewayInvoke"
#   action        = "lambda:InvokeFunction"
#   function_name = aws_lambda_function.portrait_lambda.function_name
#   principal     = "apigateway.amazonaws.com"
#   # source_arn = "${aws_api_gateway_rest_api.main.execution_arn}/*/*"
#   source_arn    = "arn:aws:execute-api:${local.region}:${local.account_id}:${aws_api_gateway_rest_api.main.id}/*/*"
# }

resource "aws_lambda_function_url" "portrait_dev" {
  function_name = aws_lambda_function.portrait_lambda.function_name
  # qualifier          = "$LATEST"
  authorization_type = "NONE"
  depends_on         = [aws_lambda_function.portrait_lambda]
...
output "portrait_lambda_base_url" {
  value = "${aws_lambda_function_url.portrait_dev.function_url}/"
}
