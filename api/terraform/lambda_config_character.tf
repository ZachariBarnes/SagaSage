resource "aws_lambda_function" "character_lambda" {
  function_name = "character_lambda"
  filename      = local.output_path
  role          = aws_iam_role.character_lambda_exec.arn
  runtime       = "nodejs18.x"
  handler       = "index.character" # <file-name>.<fucntion-name> 
...
data "archive_file" "character_lambda" {
  type = "zip"
...
# resource "aws_lambda_permission" "character_lambda_apigateway_permission" {
#   statement_id  = "AllowAPIGatewayInvoke"
#   action        = "lambda:InvokeFunction"
#   function_name = aws_lambda_function.character_lambda.function_name
#   principal     = "apigateway.amazonaws.com"
#   # source_arn = "${aws_api_gateway_rest_api.main.execution_arn}/*/*"
#   source_arn    = "arn:aws:execute-api:${local.region}:${local.account_id}:${aws_api_gateway_rest_api.main.id}/*/*"
# }

resource "aws_lambda_function_url" "character_dev" {
  function_name = aws_lambda_function.character_lambda.function_name
  # qualifier          = "$LATEST"
  authorization_type = "NONE"
  depends_on         = [aws_lambda_function.character_lambda]
...
output "character_lambda_base_url" {
  value = "${aws_lambda_function_url.character_dev.function_url}/"
}
