resource "aws_lambda_function" "signup_lambda" {
  function_name = "signUpSignIn_lambda"
  filename      = local.output_path
  role          = aws_iam_role.character_lambda_exec.arn
  runtime       = "nodejs18.x"
  handler       = "index.signUpSignInLetMeBegin" # <file-name>.<fucntion-name> 
...
# resource "aws_lambda_permission" "signup_lambda_apigateway_permission" {
...
resource "aws_lambda_function_url" "signup_dev" {
  function_name = aws_lambda_function.signup_lambda.function_name
  # qualifier          = "$LATEST"
...
output "signup_lambda_base_url" {
  value = "${aws_lambda_function_url.signup_dev.function_url}/"
}
